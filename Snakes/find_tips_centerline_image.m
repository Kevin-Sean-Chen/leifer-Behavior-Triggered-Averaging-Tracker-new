...
%             else
            %the tips is picked up by thinning but not mexican hat,
            %do nothinig
%                 TipsAndCenterlines(index).UncertainTips = [TipsAndCenterlines(index).UncertainTips; endpoints(thin_tip_index, :)];
%                 TipsAndCenterlines(index).UncertainTipsKey = [TipsAndCenterlines(index).UncertainTipsKey, 1];
        end
    end
