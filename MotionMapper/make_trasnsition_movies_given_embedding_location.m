...
for track_index = 1:length(allTracks)
    behavioral_transitions = allTracks(track_index).BehavioralTransition;
    behavior_start_frames = behavioral_transitions(:,2);
    possible_frames_in_this_track = behavior_start_frames(behavioral_transitions(:,1) == watershed_region)';
...
imagesc(xx,xx,density)
for i = 1:N
    text(selected_embedded_points(i,1), selected_embedded_points(i,2),num2str(i),'VerticalAlignment','middle','HorizontalAlignment','center','Color','b')
