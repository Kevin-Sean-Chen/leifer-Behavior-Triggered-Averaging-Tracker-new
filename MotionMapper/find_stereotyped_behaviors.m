...
Stereotyped_Behavioral_Transitions = cell(size(Embeddings));
...
for track_index = 1:length(Embeddings)
    ...
    bs_velocity = sqrt(diff(Embeddings{track_index}(:,1)).^2 + diff(Embeddings{track_index}(:,2)).^2);
    ...
    Stereotyped_Behavioral_Transitions{track_index} = [stereotyped_behavioral_transition_annotation', ...
