...
for folder_index = 1:length(folders_optotap)
    ...
    for peak_index = 1:length(peak_locations)
        ...
        current_stim_freq = PWM_frequencies(peak_locations(peak_index));
        %see if this stim_power already exists
        current_stim_index = find(stimulus_frequencies == current_stim_freq);
        if isempty(current_stim_index)
            %no entry yet
            stimulus_frequencies = [stimulus_frequencies,current_stim_freq];
            current_stim_index = length(stimulus_frequencies);
...
[stimulus_frequencies, sort_index] = sort(stimulus_frequencies);
...
for stimulus_index = 1:length(stimulus_frequencies)
    ...
    title(['Stimulus Frequency = ', num2str(stimulus_frequencies(stimulus_index)), ' Hz (n = ', num2str(track_n), ' tracks)']);
    legend('show');
    ax = gca;
    ax.FontSize = 10;
    axis([-10 10 0 35])
...
behavior_counts_for_frame = zeros(number_of_behaviors,length(stimulus_frequencies),total_window_frames);
behavior_ratios_for_frame = zeros(number_of_behaviors,length(stimulus_frequencies),total_window_frames);
...
for behavior_index = 1:number_of_behaviors
    my_colors = lines(length(stimulus_frequencies));
    figure
    hold on
    for stimulus_index = 1:length(stimulus_frequencies)
        track_n = round(mean(arrayfun(@(x) size(x{1},2), [all_behavior_transitions_for_frame{stimulus_index}])));
...
ax = gca;
ax.XTick = stimulus_frequencies;
% ax.YTick = [0 0.3 0.6];
ax.FontSize = 20;

xlabel('Stimulus Frequency (Hz)') % x-axis label
